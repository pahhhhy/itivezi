import type {FileData, FileInfo} from "@/types/chatpage/files";
import {v4 as uuid} from "uuid";
import {deleteObject, getDownloadURL, ref as storageRef, uploadBytesResumable, type FirebaseStorage  } from "firebase/storage";


export const uploadImg = async (storageInstance: FirebaseStorage, fileData: FileData, files: FileInfo[]) => {
    const id = uuid();
...
export const deleteImgFromStorage = (storageInstance: FirebaseStorage, deletedFiles: FileInfo[]) => {
...
export const splitFiles = (files: FileInfo[], content: string) => { //追加 ※1
