...
const farmarVegeList=ref<any>([])
const uniquwVegeList=ref<any>([])
...
function pushActive () {
    if(isActive.value){
        ...
        writeVegeKeys(vegeKeys.value,selectdRoadStation.value)
    }
...
function extractVegetableInfo(data: any): { [key: string]: string[] } {
    ...
    uniquwVegeList.value = Uidresult;
    return result;
}
...
async function initData(){
    ...
    farmarVegeList.value=extractVegetableInfo(vegeAllData.value)
}
initData()
...
function pushDelete(){
  isPopup.value=false
    for(let i:number=0;i<farmarVegeList.value[deleteVegeName.value].length;i++){
        deleteVegeData(deleteVegeName.value,uniquwVegeList.value[deleteVegeName.value][i],selectdRoadStation.value)
    }
...
<template>
    ...
    <div class="onwer-popup" v-if="isPopup">
        <h3>{{deleteVegeName}}は以下の{{farmarVegeList[deleteVegeName].length}}人が出品しています。本当に削除しますか？</h3>
        <div v-for="farmarName in farmarVegeList[deleteVegeName]" :key="farmarName" >
            {{ farmarName }}
        </div>
